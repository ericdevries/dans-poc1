...
public class AbrReportType extends Base {
    ...
    public static boolean isAbrReportType(Node node) {
        return "reportNumber".equals(node.getLocalName())
            && hasAttributeValue(node, "subjectScheme", DepositDatasetFieldNames.SCHEME_ABR_RAPPORT_TYPE)
            && hasAttributeValue(node, "schemeURI", DepositDatasetFieldNames.SCHEME_URI_ABR_RAPPORT_TYPE);
